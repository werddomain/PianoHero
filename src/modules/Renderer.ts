...
export class Renderer {
  ...
  private createPianoKeyboard() {
    ...
    this.PIANO_KEYS.filter(key => key.type === 'black').forEach(key => {
      ...
      const whiteKeyIndex = this.PIANO_KEYS.filter(k => 
        k.type === 'white' && 
        this.PIANO_KEYS.indexOf(k) < this.PIANO_KEYS.indexOf(key)
      ).length;
  ...
  private updateNoteElements(visibleNotes: any[]) {
  ...
  private updateNotePosition(element: HTMLElement, note: Note) {
  ...
  private getNotePosition(noteName: string): number {
    ...
    if (key.type === 'white') {
      ...
      const whiteKeyIndex = this.PIANO_KEYS.filter(
        (k, i) => k.type === 'white' && i <= keyIndex
      ).length - 1;
  ...
  public activatePianoKey(noteName: string) {
    const keyElement = document.querySelector(`[data-note="${noteName}"]`) as HTMLElement;
    if (keyElement) {
  ...
  public deactivatePianoKey(noteName: string) {
    const keyElement = document.querySelector(`[data-note="${noteName}"]`) as HTMLElement;
    if (keyElement) {
  ...
  private showHitFeedback(quality: string, noteName: string) {
